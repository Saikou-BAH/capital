...
# ── Graphiques comptes ───────────────────────────────────────────────────────
st.markdown(section_header("Vue graphique des comptes", "📊", "#3E7C51"), unsafe_allow_html=True)

g1, g2 = st.columns([3, 2])
with g1:
    st.markdown('<div class="card" style="padding:.75rem 1rem .5rem 1rem">', unsafe_allow_html=True)
    st.plotly_chart(chart_valeurs_par_compte(df_valeurs), use_container_width=True, config={"displayModeBar": False})
...
st.markdown('<div class="card" style="padding:.75rem 1rem .5rem 1rem">', unsafe_allow_html=True)
st.plotly_chart(chart_evolution_soldes_comptes(df_mvt, df_cpt), use_container_width=True, config={"displayModeBar": False})
st.markdown("</div>", unsafe_allow_html=True)
...
with f1:
    filtre_pays   = st.multiselect("Pays", PAYS_DISPONIBLES, placeholder="Tous")
with f2:
    filtre_devise = st.multiselect("Devise", DEVISES, placeholder="Toutes")
with f3:
    filtre_type   = st.multiselect("Type", TYPES_COMPTE, placeholder="Tous")
with f4:
...
if df_display.empty:
    ...
    def _render_compte_row(row):
        ...
        solde_clr = "#3E7C51" if solde > 0 else ("#B3432F" if solde < 0 else "#7F7568")
        frais_pct_val = float(row.get("frais_pct", 0) or 0)

        # Pas de colonne "Propriétaire" statique ici : la répartition dynamique
        # par investisseur (badges ci-dessous) est la source visuelle principale
        # pour savoir à qui appartient l'argent d'un compte.
        c1, c2, c3, c4 = st.columns([3.5, 2.5, 1.8, 3.5])
        with c1:
            frais_badge = (
                f'<span style="background:#F1EEF7;color:#6B5B95;font-size:.6rem;font-weight:700;'
            ...
            st.markdown(
                ...
                f'  <div>'
                f'    <div style="font-size:.87rem;font-weight:700;color:#241F19">{row["nom"]}{frais_badge}</div>'
                f'    <div style="font-size:.7rem;color:#7F7568;margin-top:.05rem">{drapeau} {row["pays"]} · {row["type_compte"]}</div>'
                f'  </div>'
                f'</div>',
                unsafe_allow_html=True,
            )
        with c2:
            st.markdown(
                f'<div style="font-size:.9rem;font-weight:800;color:{solde_clr};padding-top:.2rem;'
                f'text-align:right;font-variant-numeric:tabular-nums">{solde_fmt}</div>',
                unsafe_allow_html=True,
            )
        with c3:
            badge_html = (
                '<span style="background:#EEF5EF;color:#3E7C51;font-size:.65rem;font-weight:700;'
                'padding:.15rem .4rem;border-radius:5px">Actif</span>'
                if is_actif else
                '<span style="background:#FBEEEA;color:#B3432F;font-size:.65rem;font-weight:700;'
        ...
        with c4:
            st.markdown(f'<div class="row-comment" style="padding-top:.2rem">{str(row.get("description",""))[:70]}</div>', unsafe_allow_html=True)
        ...
        if not df_rep_cpt.empty:
            ...
            badges = "".join(
                f'<span style="background:#F5F1EA;color:#4A4238;font-size:.68rem;font-weight:600;'
    ...
    def _render_section(df_sec, title, color):
        ...
        h1, h2, h3, h4 = st.columns([3.5, 2.5, 1.8, 3.5])
        _cptrow_labels = ["Compte", "Solde estimé", "Statut", "Description"]
        _cptrow_classes = ["th", "th th-num", "th", "th"]
        for col, lbl, cls in zip([h1, h2, h3, h4], _cptrow_labels, _cptrow_classes):
            with col:
    ...
    if not df_eur.empty:
        _render_section(df_eur, f"Comptes en EUR  ({len(df_eur)})", "#B65C2E")
        st.markdown(spacer("0.5rem"), unsafe_allow_html=True)

    if not df_gnf_normal.empty:
        _render_section(df_gnf_normal, f"Comptes en GNF  ({len(df_gnf_normal)})", "#3E7C51")
        st.markdown(spacer("0.5rem"), unsafe_allow_html=True)

    if not df_gnf_frais.empty:
        _render_section(df_gnf_frais, f"Comptes GNF avec frais  ({len(df_gnf_frais)})", "#6B5B95")
        st.markdown(spacer("0.5rem"), unsafe_allow_html=True)

    if not df_other.empty:
        _render_section(df_other, f"Autres comptes  ({len(df_other)})", "#6B6155")
    ...
    with st.expander("📋 Historique du compte", expanded=False):
        ...
        if cpt_choice:
            ...
            if df_cpt_mvt.empty:
                ...
                _cpt_labels = ["Date", "Type", "Montant GNF", "Montant origine", "Taux", "Source", "Destination"]
                _cpt_classes = ["th", "th", "th th-num", "th", "th", "th", "th"]
                for col, lbl, cls in zip(cols, _cpt_labels, _cpt_classes):
                    with col:
                        st.markdown(f'<div class="{cls}">{lbl}</div>', unsafe_allow_html=True)
